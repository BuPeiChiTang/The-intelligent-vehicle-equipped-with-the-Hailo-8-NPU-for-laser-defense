...
#endif
	MPU6050_KalmanFilter kf_roll, kf_pitch, kf_xy;
	float Gyroscope_roll = 0.0f, Gyroscope_pitch = 0.0f;
...
void MPU6050_FastResetYaw_Complementary(MPU6050* sensor) {
    sensor->yaw = 0.0f;

    // 重置卡尔曼滤波器
    kf_roll.x = kf_roll.p = 0.0f;
    kf_pitch.x = kf_pitch.p = 0.0f;

    // 重置陀螺仪积分项
    Gyroscope_roll = Gyroscope_pitch = 0.0f;
}

void MPU6050_FastResetYaw_Quaternion(MPU6050* sensor) {
    sensor->yaw = 0.0f;
