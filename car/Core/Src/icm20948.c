...
/* Main Functions */
void Send_Floats9( float f1, float f2, float f3,float f4,float f5,float f6,float f7,float f8,float f9)
	 {
	     /* 使用静态缓冲区避免栈溢出风险 */
	     static char buffer[128];

	     /* 安全格式化（限制缓冲区访问） */
	     int len = snprintf(buffer, sizeof(buffer), "%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f\r\n", (float)f1, (float)f2, (float)f3,(float)f4,(float)f5, (float)f6, (float)f7,(float)f8,(float)f9);
...
float read_temperature_icm20948() {
    uint8_t temp_high = read_single_icm20948_reg(ub_0, B0_TEMP_OUT_H);
    uint8_t temp_low = read_single_icm20948_reg(ub_0, B0_TEMP_OUT_L);

    // 合并高低字节为16位有符号整数
    int16_t temp_raw = (temp_high << 8) | temp_low;

    // 转换为实际温度值
    float temperature = ((float)temp_raw / TEMP_SENSITIVITY) + TEMP_OFFSET;
