...
class ObjectDetectionUtils:
    ...
    def draw_detections(self, detections: dict, image: np.ndarray, min_score: float = 0.45, scale_factor: float = 1):
        ...
        for idx in range(detections['num_detections']):
            if scores[idx] >= min_score:
                cls = classes[idx]
                if cls not in highest_confidence_boxes or scores[idx] > highest_confidence_boxes[cls]['score']:
                    highest_confidence_boxes[cls] = {
                        'box': boxes[idx],
                        'score': scores[idx],
                        'class': cls
                    }
    ...
    # 绘制每个类别的最高置信度框
        for cls, data in highest_confidence_boxes.items():
            color = generate_color(cls)
            scaled_box = self.denormalize_and_rm_pad(data['box'], size, padding_length, img_height, img_width)
            self.draw_detection(image, scaled_box, data['class'], data['score'] * 100.0, color, scale_factor)
